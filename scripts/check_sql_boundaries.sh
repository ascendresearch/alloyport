...
# These four mixed modules are acknowledged migration debt. Remove each entry when its SQL moves
...
is_legacy_database_module() {
    case "$1" in
        ...
        crates/alloyport-server/src/storage.rs | \
...
printf 'SQL boundary check passed; legacy database modules remaining: 4\n'
