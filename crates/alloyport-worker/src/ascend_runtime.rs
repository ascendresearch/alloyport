//! Artifact, receipt, journal, device-guard, and cleanup boundary for fixed Ascend execution.

use crate::WorkerState;
use crate::ascend_smi::AscendDeviceManager;
use crate::ascend_supervisor::{
    AscendContainerEngine, AscendContainerSupervisor, AscendExecutionFacts, ContainerLogChunk,
    ContainerLogStream,
};
...
impl Debug for AscendExecutionRuntime {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AscendExecutionRuntime")
...
impl AscendExecutionRuntime {
    /// Creates one runtime bound to a worker-local device and policy composition.
    ///
    /// # Errors
    ///
    /// Returns an error when the worker or device identity is empty.
    pub fn new(
        ...
        if worker_id.trim().is_empty() {
            return Err(ExecutionRuntimeError::InvalidConfiguration(
                "Ascend worker identity must be nonempty",
            ));
        }
        let device = supervisor.device().clone();
        let environment = supervisor.environment().clone();
        Ok(Self {
            worker_id,
            device,
            environment,
    ...
    #[must_use]
    pub const fn device(&self) -> &alloyport_core::AcceleratorDevice {
        &self.device
    }

    #[must_use]
    pub const fn environment(&self) -> &crate::ascend::AscendEnvironmentFacts {
        &self.environment
    }
    ...
    /// # Errors
    ///
    /// Returns an error for journal, device, supervisor, Artifact, or cleanup failures.
    pub async fn run(
        &self,
        state: &WorkerState,
        attempt_id: &str,
    ...
    /// Runs and publishes terminal Artifacts before committing terminal journal state.
    ///
    /// # Errors
    ///
    ...
    /// Runs while forwarding bounded best-effort output observations.
    ///
    /// # Errors
    ///
    ...
    where
        F: FnMut(ExecutionObservation) + Send,
    {
        let _claim = AscendAttemptClaim::acquire(Arc::clone(&self.active_attempts), attempt_id)?;
        let attempt = state
        ...
        let mut events = vec![producer_event(
            ...
            Event::CommandStarted {
                ...
                description: Some("policy-bound Ascend fixture".into()),
            },
        )];
        let execution = self
            .supervisor
        ...
        let post_observation =
            observe_exact(self.device_manager.as_ref(), &self.device.device_id).await?;
        ...
        let observation = state
            ...
            .ok_or_else(|| {
                ExecutionRuntimeError::Backend(BackendError::integrity(format!(
                    "running attempt {attempt_id} lacks durable Ascend preflight evidence"
                )))
            })?;
        if observation.device_id != self.device.device_id
            || lease.device_id != self.device.device_id
        {
            return Err(ExecutionRuntimeError::Backend(BackendError::integrity(
                format!("running attempt {attempt_id} has Ascend evidence for a different device"),
            )));
        }
    ...
    async fn persist(
    ...
    ) -> Result<AscendPersistedExecution, ExecutionRuntimeError> {
        ...
        .await?;
        let receipt = serde_json::to_vec(&AscendRunReceipt {
            schema_version: 2,
...
async fn observe_exact(
    manager: &dyn AscendDeviceManager,
    device_id: &str,
...
async fn active_lease(
    ...
    state
        ...
        .ok_or_else(|| {
            ExecutionRuntimeError::Backend(BackendError::integrity(format!(
                "attempt {attempt_id} lost its durable Ascend device lease"
            )))
        })
}
...
fn append_terminal_events(
    worker_id: &str,
    input: &ExecutorInput,
    persisted: &AscendPersistedExecution,
    events: &mut Vec<ProducerEvent>,
) {
...
struct AscendPersistedExecution {
    finished: StoredFinished,
...
struct AscendRunReceipt<'a> {
    ...
    device: &'a alloyport_core::AcceleratorDevice,
    environment: &'a crate::ascend::AscendEnvironmentFacts,
    lease: &'a DeviceLease,
...
struct AscendAttemptClaim {
    attempts: Arc<Mutex<BTreeSet<String>>>,
    attempt_id: String,
}

impl AscendAttemptClaim {
    fn acquire(
        attempts: Arc<Mutex<BTreeSet<String>>>,
        attempt_id: &str,
    ) -> Result<Self, ExecutionRuntimeError> {
...
impl Drop for AscendAttemptClaim {
    fn drop(&mut self) {
