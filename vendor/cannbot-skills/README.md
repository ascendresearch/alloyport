...
| 优化 Triton 算子 | "请使用 triton-optimizer 优化当前目录的 Triton 算子，并从 baseline 开始记录每轮结果" | Plugin: `triton-optimizer` |
| 转换 PyTorch 算子 | "请使用 triton-optimizer 将 /path/to/op.py 转成 Triton Ascend NPU 实现并验证" | Plugin: `triton-optimizer` |
...
| **Triton Optimizer** | 已有 Triton Ascend NPU 算子的 baseline/round 迭代优化，以及 PyTorch 算子到 Triton Ascend NPU 的转换验证 | triton-optimizer | triton-npu-optimize · triton-npu-convert-pytorch-operator · ascend-npu-run-eval |
